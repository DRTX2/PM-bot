...
file_path = '/home/david/Desktop/personal/docker-programs/n8n/bot/Reporte PDF Semanal.json'

with open(file_path, 'r') as f:
    wf = json.load(f)

# 1. Remove connections from PR nodes to Merge Reporte
if 'GitHub Back PRs' in wf['connections']:
    del wf['connections']['GitHub Back PRs']
if 'GitHub Front PRs' in wf['connections']:
    del wf['connections']['GitHub Front PRs']

# 2. Update code in 'Build Report Data'
for node in wf['nodes']:
    if node['name'] == 'Build Report Data':
        code = node['parameters']['jsCode']
        
        # Replace the part where it reads PRs or just inject the direct fetch
        # Let's replace the whole code with a version that fetches directly
        
        new_code = """const items = $input.all().map(i => i.json);
...
// --- NEW: Fetch PRs directly to avoid split execution ---
const backPRs = $('GitHub Back PRs').all().map(i => i.json);
const frontPRs = $('GitHub Front PRs').all().map(i => i.json);
